...
Compiler.prototype = {
  ...
  visitTag: function(tag /*interpolated*/){
    ...
    var name = tag.name
      , pp = this.pp
      , self = this
      , render = self.options.tagModels[tag.name].render
      , renderBefore = self.options.tagModels[tag.name].renderBefore
      , renderAfter = self.options.tagModels[tag.name].renderAfter;
    function bufferName() {
      // if (interpolated) self.bufferExpression(tag.expr);
      if (renderBefore) {
        self.tagBuffer(renderBefore(tag.attrs))
      } else {
    ...
    // } else {
      ...
      //закрытие тега
      if (renderAfter) {
        this.tagBuffer(renderAfter(tag.attrs))
      }
