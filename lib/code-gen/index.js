...
function Compiler(node, options) {
  ...
  this.runtimeFunctionsUsed = ['attr'];
...
Compiler.prototype = {
  ...
  tagBuffer: function (src) {
    // if (isConstant(src)) {
    //   return this.buffer(toConstant(src) + '')
    // }
    //TODO разобраться с after tag
    if (!src) {
        console.debug('src is undefined')
        return
    }
    if (this.lastBufferedIdx == this.buf.length && this.bufferedConcatenationCount < 100) {
      this.bufferedConcatenationCount++;
  ...
  //возможно ли писать более однонго текста в блоке?
  getText: function(block) {
      if (! block.nodes.length) {
          return ""
      }
      return block.nodes.reduce((acc, node) => {
          if (node.type === 'Text') {
              acc += "\"" + node.val + "\""
          }
          if (node.type === 'Code') {
              console.log(this.getCode(node), '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
              console.log(this.getCode(node), '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
              console.log(this.getCode(node), '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
              console.log(this.getCode(node), '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
              console.log(this.getCode(node), '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
              ...
              console.log(this.getCode(node), '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
              console.log(this.getCode(node), '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
              acc += " + " + this.getCode(node)
            //   acc.push(this.getCode(node))
          }
          console.log(acc, ')))))')
          return acc
      }, "")
  ...
  visitTag: function(tag /*interpolated*/){
    this.indents++;
    if (! this.options.tagModels[tag.name]) {
      return this.indents--
    }
    var name = tag.name
      , pp = this.pp
      , self = this
      , render = self.options.tagModels[tag.name].render.bind(self.options.tagModels[tag.name]);
    // function bufferName() {
    //   // if (interpolated) self.bufferExpression(tag.expr);
    //   if (renderBefore) {
    //     self.tagBuffer(render(tag.attrs,))
    //   } else {
    //     //buffer взять строку и запушить в this.buf
    //     self.tagBuffer(render(tag.attrs))
    //   }
    // }
    console.log(tag, tag.block.nodes)
    ...
    // const a = this.visitAttributes(tag.attrs, this.attributeBlocks(tag.attributeBlocks))
    //TODO починить переменные в тексте
    const _text = self.getText(tag.block)
    console.log(_text, '&&&&&')
    const beforeTag = render(tag.attrs, _text, false, { childs: tag.block.nodes })
     if (beforeTag) {
        self.tagBuffer(beforeTag)
     }
     console.log(render(tag.attrs, _text, null, { childs: tag.block.nodes }))
    self.tagBuffer(render(tag.attrs, _text, null, { childs: tag.block.nodes }))
    //pretty print
    if (pp && !tag.isInline)
      this.prettyIndent(0, true);
    ...
    // } else {
      ...
      if (pp && !tag.isInline && WHITE_SPACE_SENSITIVE_TAGS[tag.name] !== true && !tagCanInline(tag))
        this.prettyIndent(0, true);
      //закрытие тега
      const afterTag = render(tag.attrs, _text, true, { childs: tag.block.nodes })
     if (afterTag) {
        self.tagBuffer(afterTag)
     }
   ...
   * @api public
   */
  //пересобрать в виде key=val || key=expression(): val
